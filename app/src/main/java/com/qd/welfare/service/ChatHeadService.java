package com.qd.welfare.service;
            ...
            try {
                if (MainActivity.isApplicationBroughtToBackground(getApplicationContext())) {
                    if (exitTime == 0) {
                        ...
                        VideoResultInfo.VideoIndexInfo videoIndexInfo = JSON.parseObject(jsonStr, VideoResultInfo.VideoIndexInfo.class);
                        if (count == 0) {
                            //第一次是5分钟提示
                            if (System.currentTimeMillis() - exitTime > 5 * 60 * 1000) {
                                if (!viewIsadded && videoIndexInfo != null) {
                                    title.setText(videoIndexInfo.getVideo().get(0).getTitle());
                                    Glide.with(ChatHeadService.this).load(App.commonInfo.getFile_domain() + videoIndexInfo.getVideo().get(0).getThumb()).asBitmap().centerCrop().into(image);
