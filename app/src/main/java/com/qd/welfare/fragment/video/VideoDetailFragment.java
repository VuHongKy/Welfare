...
public class VideoDetailFragment extends BaseBackFragment {
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            videoId = getArguments().getInt("id", 0);
        }
    }
    ...
    private void initView() {
        adapter = new VideoRecommendAdapter(getContext(), list);
        initHeaderView();
    ...
    private void initHeaderView() {
        View headerView = LayoutInflater.from(getContext()).inflate(R.layout.fragment_video_detail_header, null);
    ...
    private void bindHeaderView(VideoDetailInfo info) {
        Glide.with(getContext()).load(App.commonInfo.getFile_domain() + info.getThumb()).centerCrop().into(image);
